...
logger.info(f"FFprobe path: {FFPROBE}")
...
def get_gemini_model():
    ...
    # gemini-1.5-flash directly - no test ping to waste free tier quota
    model = genai.GenerativeModel("gemini-1.5-flash")
    logger.info("Gemini model ready: gemini-1.5-flash")
    return model
  ...
  "chapters": [
    ...
    for attempt in range(3):
        try:
            ...
            if start == -1 or end == 0:
                raise Exception(f"No JSON in response: {text[:150]}")
            text = text[start:end]
            data = json.loads(text)
            total_words = sum(len(c["text"].split()) for c in data["chapters"])
            logger.info(f"Script OK: {total_words} words")
            return data
        except Exception as e:
            logger.warning(f"Script attempt {attempt+1} failed: {e}")
            wait = 65 * (attempt + 1)
            logger.info(f"Waiting {wait}s before retry (quota cooldown)...")
            time.sleep(wait)

    raise Exception("Failed to generate script after 3 attempts")
